/*
 * Lab 05 - shared message layout between M55 (listener) and M4
 * (accelerometer monitor). Reused unchanged from Lab 03 -- only one
 * message type ever flows in this lab (M4 -> M55: "the motion state
 * just changed"), so unlike a general-purpose protocol there is no
 * type/command tag here.
 */

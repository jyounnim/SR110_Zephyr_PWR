/*
 * Lab 03 - M4 side: continuously monitor the onboard accelerometer
 * (mc3479 on I2C1) and notify M55 over mbox ONLY when the motion state
 * changes (still <-> active), not on every sample.
 *
 * This is the third kind of "wake" in this curriculum, after Lab 01's
 * WFI (shallow, resumes exactly where it left off) and Lab 02's watchdog
 * reset (deep, restarts main() from scratch): here M55 stays fully alive
 * the whole time and is simply woken by another core's message, with no
 * restart of any kind.
 *
 * Hardware-confirmed (2026-09-21): shaking the board produces a clean
 * STILL/ACTIVE alternation on the M55 side with no missed or duplicate
 * events, using ACCEL_THRESHOLD_MILLI_G = 2000 as-is (no retuning
 * needed for this board/mounting).
 *
 * Design notes:
...
int main(void)
{
	printk("\n=== Lab 03 (M4): accelerometer monitor, notify M55 on state change ===\n");
